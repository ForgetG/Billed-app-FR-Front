...
import Deds from "../containers/Deds.js"
import NewBill from "../containers/NewBill.js"
import NewDED from "../containers/NewDED.js"
...
export default () => {
  ...
  window.onNavigate = (pathname) => {
    ...
    } else if (pathname === ROUTES_PATH['Deds']) {
      rootDiv.innerHTML = ROUTES({ pathname, loading: true });
      const divIcon1 = document.getElementById('layout-icon1');
      const divIcon2 = document.getElementById('layout-icon2');
      divIcon1.classList.add('active-icon');
      divIcon2.classList.remove('active-icon');
      const deds = new Deds({ document, onNavigate, store, localStorage });
      deds.getDeds().then(data => {
        rootDiv.innerHTML = DedsUI({ data });
        const divIcon1 = document.getElementById('layout-icon1');
        const divIcon2 = document.getElementById('layout-icon2');
        divIcon1.classList.add('active-icon');
        divIcon2.classList.remove('active-icon');
        new Deds({ document, onNavigate, store, localStorage });
      }).catch(error => {
        rootDiv.innerHTML = ROUTES({ pathname, error });
      });
    } else if (pathname === ROUTES_PATH['NewDED']) {
      rootDiv.innerHTML = ROUTES({ pathname, loading: true });
      new NewDED({ document, onNavigate, store, localStorage });
      const divIcon1 = document.getElementById('layout-icon1');
      const divIcon2 = document.getElementById('layout-icon2');
      divIcon1.classList.remove('active-icon');
      divIcon2.classList.add('active-icon');
    ...
    } else if (pathname === ROUTES_PATH['Register']) {
      rootDiv.innerHTML = ROUTES({ pathname });
      new Register({ document, localStorage, onNavigate, store });
    }
  };
  ...
  if (window.location.pathname === "/" && window.location.hash === "") {
    ...
    if (window.location.hash === ROUTES_PATH['Bills']) {
      ...
      divIcon1.classList.remove('active-icon');
      divIcon2.classList.add('active-icon');
    } else if (window.location.hash === ROUTES_PATH['Deds']) {
      rootDiv.innerHTML = ROUTES({ pathname: window.location.hash, loading: true });
      const divIcon1 = document.getElementById('layout-icon1');
      const divIcon2 = document.getElementById('layout-icon2');
      divIcon1.classList.add('active-icon');
    ...
    } else if (window.location.hash === ROUTES_PATH['NewDED']) {
      rootDiv.innerHTML = ROUTES({ pathname: window.location.hash, loading: true });
      new NewDED({ document, onNavigate, store, localStorage });
      const divIcon1 = document.getElementById('layout-icon1');
      const divIcon2 = document.getElementById('layout-icon2');
